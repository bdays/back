  ...
  const list = await asyncWrapper(
    BdayModel.findAll({
      attributes: [
        ...
        [sequelize.literal('extract(epoch from "BdaySchedules".date at time zone \'utc\')'), 'date'],
        [sequelize.literal('extract(epoch from "BdaySchedules"."updatedAt" at time zone \'utc\')'), 'updatedAt'],
      ],
